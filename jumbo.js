    ...
    const categories = await page.evaluate(() => {
        ...
        categoryElements.forEach((categoryElement) => {
            ...
            offerElements.forEach((offerElement) => {
                ...
                const twoLinesdeal = [
                    offerElement
                        .querySelector('.tag .upper')
                        ?.textContent.trim(),
                    offerElement
                        .querySelector('.tag .lower')
                        ?.textContent.trim(),
                ]

                const imageUrl =
                    offerElement.querySelector('.card-image img')?.src
...
async function main() {
    ...
    await writeFile('./out/jumbo.json', JSON.stringify(offers, null, 4))
}
